namespace PlayerUnknown.Engine.Models
{
    using System;
    ...
    public struct AActor
    {
        ...
        {
            if (AActor.g_pLocalPlayer != IntPtr.Zero)
            {
                var localplayer = M.Read<AActor>(AActor.g_pLocalPlayer);
                localplayer.BasePointer = AActor.g_pLocalPlayer;
                return localplayer;
            }

            // AActor.g_pLocalPlayer = G.OwningGameInstance.LocalPlayer.PlayerController.pLocalPlayer;
            return M.Read<AActor>(AActor.g_pLocalPlayer);
        }
    }
    ...
    public unsafe struct FString
    {
        ...
        {
            fixed (sbyte* pName = this.arrName)
    ...
    public struct ULocalPlayer
    {
        ...
        {
            M.Write<Vector3>(vecLocation, this.pBase + 0x70/*Location*/);
        }
    ...
    public struct APlayerController
    {
        ...
        public APlayerCameraManager PlayerCameraManager
        {
            get
            {
                return M.Read<APlayerCameraManager>(this.pPlayerCameraManager);
            }
        }
    }
    ...
    public struct TArray<T> where T : struct
    {
        ...
        public IntPtr this[int nIndex]
        {
            get
            {
                return this.pData + nIndex * IntPtr.Size;
            }
        }
        public T ReadValue(int nIndex, bool bDeref)
        {
            IntPtr ptrData = this.pData + nIndex * IntPtr.Size;
    ...
    public unsafe struct TStaticIndirectArrayThreadSafeRead
    {
        ...
        public fixed long Chunks[TStaticIndirectArrayThreadSafeRead.ChunkCount];
        ...
        public List<byte[]> ChunkData
        {
            get
            {
                ...
                fixed (long* arrChunkPointers = this.Chunks)
                    for (int nChunk = 0; nChunk < TStaticIndirectArrayThreadSafeRead.ChunkCount; nChunk++)
                        listChunkdata.Add(M.Read((IntPtr) arrChunkPointers[nChunk], TStaticIndirectArrayThreadSafeRead.ElementsPerChunk * IntPtr.Size));
        ...
        public IntPtr this[int nIndex]
        {
            get
            {
                fixed (long* chunks = this.Chunks)
                {
                    var pChunk = (IntPtr) chunks[nIndex / TStaticIndirectArrayThreadSafeRead.ElementsPerChunk];
                    var arrChunkData = this.ChunkData[nIndex / TStaticIndirectArrayThreadSafeRead.ElementsPerChunk];
                    return (IntPtr) BitConverter.ToInt64(arrChunkData, nIndex % TStaticIndirectArrayThreadSafeRead.ElementsPerChunk * IntPtr.Size);
                }
            }
        }
        public string[] DumpNames()
        {
            string[] arrNames = new string[TStaticIndirectArrayThreadSafeRead.MaxTotalElements];
            var arrChunkData = this.ChunkData;

            for (int nIndex = 0; nIndex < TStaticIndirectArrayThreadSafeRead.MaxTotalElements; nIndex++)
            {
                byte[] arrCurrentChunkData = arrChunkData[nIndex / TStaticIndirectArrayThreadSafeRead.ElementsPerChunk];

                IntPtr pName = (IntPtr) BitConverter.ToInt64(arrCurrentChunkData, nIndex % TStaticIndirectArrayThreadSafeRead.ElementsPerChunk * IntPtr.Size);
